...
闭包解释:
    ...
    格式:
        def 外部函数名(形参列表):
            ...
            return 内部函数名
    前提条件:
        1.机器学习概述. 有嵌套.         外部函数嵌套内部函数
        2. 有引用.         内部函数使用外部函数的变量
        3. 有返回.         外部函数中, 返回 内部函数名(对象)

细节:
    1.机器学习概述. 函数名 和 函数名() 是两个概念, 前者表示 函数对象, 后者表示 调用函数, 获取返回值.
"""
...
* 场景1: ==**无参无返回值的原函数**==
  ...
  def my_decorator(fn_name):
      # 1.机器学习概述.1.机器学习概述 定义内部函数, 其格式必须和 被装饰的原函数 保持一致.
      def fn_inner():                 # 有嵌套
          # 1.机器学习概述.2 添加提示信息(额外功能)
          print('正在努力计算中...')     # 有额外功能
          # 1.机器学习概述.3 调用原函数.
          fn_name()                   # 有引用
      # 1.机器学习概述.4 返回内部函数(对象)
      return fn_inner                 # 有返回
