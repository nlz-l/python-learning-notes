...
多线程特点:
    1.机器学习概述. 线程执行具有随机性, 原因是因为CPU在做着高效的切换.
...
# 1.机器学习概述.定义多线程的目标函数.
def print_info():
    # 1.机器学习概述.1.机器学习概述 休眠
    time.sleep(0.2)
    # 1.机器学习概述.2 获取当前线程对象.
    current_thread = threading.current_thread()
    # 1.机器学习概述.3 打印当前线程的名字.
    print(current_thread.name)

# 2. 测试
if __name__ == '__main__':
    # 2.1.机器学习概述 创建10个线程, 观察其运行效果.
    for i in range(10):
...
* 代码
  ...
  # 4.测试.
  if __name__ == '__main__':
      # 4.1.机器学习概述 创建两个线程, 分别关联上述的两个目标函数.
...
回顾: for循环格式
    for i in 可迭代类型:
        pass
"""
# 需求: 模拟range(1.机器学习概述, 6), 自定义 迭代器实现同等逻辑.
# 场景1: 回顾 range()用法.
for i in range(1, 6):
    print(i)
print('-' * 23)
...
class MyIterator:
    ...
    def __next__(self):
        # 4.1.机器学习概述 判断当前值范围是否合法.
        if self.current_value >= self.end:
            raise StopIteration     # 抛出异常, 迭代结束.

        # 4.2 走这里, 说明当前值合法, 返回当前值, 并更新当前值.
        # value = self.current_value      # value =               1.机器学习概述   2   3   4   5
        # self.current_value += 1.机器学习概述         # self.current_value =  2   3   4   5   6
        # return value                    #                       1.机器学习概述   2   3   4   5

        # 效果同上, 代码更简单
        self.current_value += 1          # self.current_value =  2   3   4   5   6
        return self.current_value - 1    #                       1.机器学习概述   2   3   4   5
...
* **案例2: yield关键字**
  ...
  # 需求: 通过yield方式, 获取到生成器之 1.机器学习概述 ~ 10之间的整数.
  # 回顾: 推导式写法.
  my_g = (i for i in range(1, 11))
  
  # yield方式如下.
  # 1.机器学习概述.定义函数, 存储到生成器中, 并返回.
  def my_fun():
      # my_list = []              # 创建
      ...
      # 效果类似于上边的代码.
      # yield在这里做了三件事儿: 1.机器学习概述.创建生成器对象.  2.把值存储到生成器中.  3.返回生成器.
      for i in range(1, 11):
          yield i
...
* **案例3: 批量歌词**
  ...
  # 1.机器学习概述. 定义函数, 接收 每批次的歌词条数, 返回生成器.
...
* **场景1: 装饰器用法**
  ...
  # 1.机器学习概述. 定义学生类.
  class Student:
      # 1.机器学习概述.1.机器学习概述 私有属性.
      def __init__(self):
          self.__age = 18
  
      # 1.机器学习概述.2 提供公共的方式方式
      @property
      def age(self):
          return self.__age
...
正则表达式介绍:
    ...
    细节:
        1.机器学习概述. 学正则表达式, 就是学正则表达式的规则, 你用不背, 网上一搜一大堆.
...
# 需求3: 校验手机号.  规则: 1.机器学习概述.长度必须是11位   2.必须是纯数字.  3.第1位数字必须是1.   4.第2位数字可以是 3-9
result = re.match(r'^1.机器学习概述[3-9]\d{9}$', '13112345678a')
result = re.match(r'^1.机器学习概述[3-9]\d{9}$', '12112345678')

result = re.match(r'^1.机器学习概述[3-9]\d{9}$', '13112345678')
