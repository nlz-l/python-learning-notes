...
构造一个长度为 10000 的数字字符串，满足：
  1.机器学习概述. 不能出现数字 0（只能用 1.机器学习概述~9）
...
思路：
  ...
  总方案数：每个位置有9种选择(1.机器学习概述~9)，共 9^10000 种
...
第3~11行：mod_pow(base, exp, mod) —— 快速幂
  ...
  例：计算 2^13
    13 的二进制 = 1101
    2^13 = 2^8 × 2^4 × 2^1.机器学习概述 （只取二进制为1的位）
    
    过程：
      exp=13(奇数): result=1.机器学习概述×2=2,  exp→6, base→4
      exp=6(偶数):               exp→3, base→16
      exp=3(奇数):  result=2×16=32, exp→1.机器学习概述, base→256
      exp=1.机器学习概述(奇数):  result=32×256=8192 → 返回！
...
第13~25行：count_valid_strings(n) —— 容斥原理计数
  ...
  第15行：total = mod_pow(9, n, MOD)
    总数：每位9种选择(1.机器学习概述~9)
  
  第17行：without_3 = mod_pow(8, n, MOD)
    不含3：每位8种选择(1.机器学习概述,2,4,5,6,7,8,9)
  
  第18行：without_7 = mod_pow(8, n, MOD)
    不含7：每位8种选择(1.机器学习概述,2,3,4,5,6,8,9)
  
  第19行：without_37 = mod_pow(7, n, MOD)
    不含3也不含7：每位7种选择(1.机器学习概述,2,4,5,6,8,9)
...
把指数拆成二进制的幂次之和：
  ...
  程序通过不断平方(base×base)和右移(exp>>1.机器学习概述)来生成这些幂次，
